...
The slot `@proxy` returns the underlying scidb proxy object and the operation `[]` can be used to download data from a SciDB array as an R data.frame.
...
For two dimensional arrays, the scidbst package comes with function to download data as RasterBrick with `as()`. We need to create a scidbstproxy first and then plot the time series length of individual pixels, the change date, and the change magnitude. The length of the time series clearly shows a pattern due to Landsat7's failure of the scan line corrector.
